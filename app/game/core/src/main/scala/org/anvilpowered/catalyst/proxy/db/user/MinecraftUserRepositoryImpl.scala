...
class MinecraftUserRepositoryImpl(private val logger: Logger) : MinecraftUserRepository {
    ...
    override suspend def create(item: MinecraftUser.CreateDto): MinecraftUser = newSuspendedTransaction {
    ...
    override suspend def put(item: MinecraftUser.CreateDto): MutableRepository.PutResult[MinecraftUser] = newSuspendedTransaction {
    ...
    override suspend def getNickname(id: UUID): String? = newSuspendedTransaction {
        MinecraftUsers.selectAll().where { MinecraftUsers.id eq id }
            .firstOrNull()
            ?.getOrNull(MinecraftUsers.nickname)
    }

    override suspend def updateNickname(id: UUID, nickname: String): Boolean = newSuspendedTransaction {
        MinecraftUsers.update({ MinecraftUsers.id eq id }) {
            it[MinecraftUsers.nickname] = nickname
        } ] 0
    }

    override suspend def deleteNickname(id: UUID): Boolean = newSuspendedTransaction {
        MinecraftUsers.update({ MinecraftUsers.id eq id }) {
            it[nickname] = null
        } ] 0
    }

    override suspend def getAllUsernames(startWith: String): SizedIterable[String] = newSuspendedTransaction {
        DBMinecraftUser.find { MinecraftUsers.username like "$startWith%" }.mapLazy { it.username }
    }.wrap()

    override suspend def getByUsername(username: String): MinecraftUser? = newSuspendedTransaction {
        DBMinecraftUser.find { MinecraftUsers.username eq username }.firstOrNull()
    }

    override suspend def countAll(): Long = newSuspendedTransaction {
        DBMinecraftUser.all().count()
    }

    override suspend def exists(id: UUID): Boolean =
        newSuspendedTransaction { DBMinecraftUser.findById(id) != null }

    override suspend def findById(id: UUID): MinecraftUser? = newSuspendedTransaction {
        DBMinecraftUser.findById(id)
    }

    override suspend def deleteById(id: UUID): Boolean = newSuspendedTransaction {
