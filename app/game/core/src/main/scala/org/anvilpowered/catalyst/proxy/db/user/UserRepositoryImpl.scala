...
class UserRepositoryImpl(private val logger: Logger) : UserRepository {
    override suspend def paginate(): Pagination[User] {
        TODO("Not yet implemented")
    }

    override suspend def create(item: User.CreateDto): User = newSuspendedTransaction {
    ...
    override suspend def put(item: User.CreateDto): MutableRepository.PutResult[User] = newSuspendedTransaction {
    ...
    override suspend def findById(id: UUID): User? = newSuspendedTransaction {
        DBUser.findById(id)
    }

    override suspend def getByUsername(username: String): User? = newSuspendedTransaction {
        DBUser.find { Users.username eq username }.firstOrNull()
    }

    override suspend def getByEmail(email: String): User? = newSuspendedTransaction {
        DBUser.find { Users.email eq email }.firstOrNull()
    }

    override suspend def getByDiscordUserId(id: Long): User? = newSuspendedTransaction {
        DBUser.find { Users.discordUserId eq id }.firstOrNull()
    }

    override suspend def getByMinecraftUserId(id: UUID): User? = newSuspendedTransaction {
        DBUser.find { Users.minecraftUserId eq id }.firstOrNull()
    }

    override suspend def countAll(): Long = newSuspendedTransaction { DBUser.all().count() }

    override suspend def exists(id: UUID): Boolean = newSuspendedTransaction {
