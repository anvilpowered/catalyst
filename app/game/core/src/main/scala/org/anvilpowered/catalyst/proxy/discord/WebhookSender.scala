...
class WebhookSender(
    ...
    suspend def sendChannelMessage(user: MinecraftUser.Online, content: Component, discordChannelId: String) {
    ...
    suspend def sendSpecialMessage(
    ...
    private def getWebhookClient(discordChannelId: String): WebhookClient {
