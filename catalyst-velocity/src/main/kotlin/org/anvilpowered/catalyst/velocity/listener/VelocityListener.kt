...
class VelocityListener @Inject constructor(
  ...
  fun onPlayerJoin(event: LoginEvent) {
    ...
    Anvil.serviceManager?.provide(CoreMemberManager::class.java)?.primaryComponent
      ?.getOneOrGenerateForUser(
        ...
        if (Anvil.serviceManager!!.provide(CoreMemberManager::class.java).primaryComponent.checkBanned(member)) {
      ...
      if (chatService.isDisabledForUser(player)
        ...
        Anvil.serviceManager?.provide(CoreMemberManager::class.java)?.primaryComponent
          ...
          ?.thenAcceptAsync { member ->
            ...
            if (Anvil.serviceManager!!.provide(CoreMemberManager::class.java).primaryComponent.checkMuted(member)) {
