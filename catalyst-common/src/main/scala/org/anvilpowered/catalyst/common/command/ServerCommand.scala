...
class ServerCommand[TPlayer : TCommandSource, TCommandSource] @Inject constructor(
    ...
    def execute(context: CommandContext[TCommandSource]): Int {
    ...
    private def commenceConnection(player: TPlayer, server: String) {
        val userName = userService.getUserName(player)
    ...
    private def testChannel(player: TPlayer, server: String) {
    ...
    def sendServers(context: CommandContext[TCommandSource]): Int {
    ...
    private def alreadyConnected(targetServer: String, player: TPlayer): Int {
        Component.text()
            .append(pluginInfo.prefix)
