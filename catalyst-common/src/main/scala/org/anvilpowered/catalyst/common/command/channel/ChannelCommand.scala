...
class ChannelCommand[TPlayer : TCommandSource, TCommandSource] @Inject constructor(
    ...
    private def exists(channelId: String): Boolean = registry.getOrDefault(CatalystKeys.CHAT_CHANNELS).any { it.id == channelId }

    def setAlias(context: CommandContext[TCommandSource], channelId: String, playerClass: Class[*]): Int {
    ...
    def set(context: CommandContext[TCommandSource], playerClass: Class[*]): Int {
    ...
    def abortEdit(context: CommandContext[TCommandSource]): Int {
        val source = context.source
    ...
    def list(context: CommandContext[TCommandSource], playerClass: Class[*]): Int {
    ...
    private inner class Renderer : Pagination.Renderer.RowRenderer[Component] {
        ...
        override def renderRow(value: Component?, index: Int): MutableCollection[Component] {
    ...
    private def channelInfo(channelId: String, active: Boolean): Component {
    ...
    def startEdit(context: CommandContext[TCommandSource]): Int {
        val source = context.source
    ...
    def editProperty(context: CommandContext[TCommandSource]): Int {
        val source = context.source
    ...
    def commit(context: CommandContext[TCommandSource]): Int {
        val source = context.source
    ...
    def info(context: CommandContext[TCommandSource]): Int {
    ...
    private def buildInfo(channelId: String): Component {
        if (!exists(channelId)) {
    ...
    private def editableInfo(channelId: String): Component {
        if (!exists(channelId)) {
    ...
    private def infoBar(name: String): Component {
        return Component.text()
    ...
    private def editBar(name: String): Component {
        return Component.text()
            .append(Component.text("========= ").color(NamedTextColor.DARK_GREEN))
    ...
    private def basicProperty(name: String, value: String): Component {
        return Component.text()
    ...
    private def editableProperty(name: String, value: String): Component {
    ...
    private def TCommandSource.uuid() = userService.getUUID(this as TPlayer)
}
