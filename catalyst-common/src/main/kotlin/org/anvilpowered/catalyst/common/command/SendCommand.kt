...
class SendCommand<TString, TPlayer : TCommandSource, TCommandSource> @Inject constructor(
  ...
  fun execute(context: CommandContext<TCommandSource>): Int {
    ...
    if (target == "_a") {
      CompletableFuture.runAsync {
        ...
        for (p in userService.onlinePlayers) {
          if (locationService.getServerForName(serverName).map { s: BackendServer -> s.connect(p).join() }.orElse(false)) {
            ++total
            textService.builder()
              .append(pluginInfo.prefix)
    ...
    if (target.startsWith("_")) {
      ...
      for (p in userService.onlinePlayers) {
        ...
        if (targetServer == locationService.getServer(targetUserName).map { obj: Named -> obj.name }
            .orElse(null)) {
          server.connect(p).thenAccept { result: Boolean ->
            if (result) {
              textService.builder()
                .append(pluginInfo.prefix)
  ...
  private fun connect(server: BackendServer?, source: TCommandSource, target: TPlayer): Int {
    ...
    server.connect(target).thenApply { result: Boolean ->
      if (result) {
        textService.builder()
          .append(pluginInfo.prefix)
          .green().append("You have been sent to ")
