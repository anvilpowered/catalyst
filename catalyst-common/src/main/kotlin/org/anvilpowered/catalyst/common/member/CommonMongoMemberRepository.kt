...
class CommonMongoMemberRepository : CommonMemberRepository<ObjectId, Datastore>(), BaseMongoRepository<Member<ObjectId>>, MongoMemberRepository {

    override fun getOneOrGenerateForUser(userUUID: UUID, userName: String, ipAddress: String, flags: BooleanArray): CompletableFuture<Member<ObjectId>?> {
    ...
    override fun getOneForUser(userName: String): CompletableFuture<Member<ObjectId>?> {
        return getOne(asQuery(userName))
    }

    override fun getForIpAddress(ipAddress: String): CompletableFuture<List<Member<ObjectId>>> {
    ...
    override fun asQuery(userUUID: UUID): Query<Member<ObjectId>> {
        return asQuery().filter(Filters.eq("userUUID", userUUID))
    }

    override fun asQuery(userName: String): Query<Member<ObjectId>> {
        return asQuery().filter(Filters.eq("userName", userName))
    }

    override fun asQueryForIpAddress(ipAddress: String): Query<Member<ObjectId>> {
        return asQuery().filter(Filters.eq("ipAddress", ipAddress))
    }

    private fun ban(query: Query<Member<ObjectId>>, endUtc: Instant, reason: String): CompletableFuture<Boolean> {
    ...
    override fun unBan(query: Query<Member<ObjectId>>): CompletableFuture<Boolean> {
        return update(query, set("banned", false))
    }

    override fun mute(query: Query<Member<ObjectId>>, endUtc: Instant, reason: String): CompletableFuture<Boolean> {
        val updateOperators: MutableList<UpdateOperator> = ArrayList()
    ...
    override fun unMute(query: Query<Member<ObjectId>>): CompletableFuture<Boolean> {
        return update(query, set("muted", false))
    }

    override fun setNickName(query: Query<Member<ObjectId>>, nickName: String): CompletableFuture<Boolean> {
        return update(query, set("nickName", nickName))
    }

    override fun deleteNickName(query: Query<Member<ObjectId>>): CompletableFuture<Boolean> {
