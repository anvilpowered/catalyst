...
class DeleteNicknameCommand<TString, TPlayer : TCommandSource, TCommandSource> @Inject constructor(
  ...
  fun execute(context: CommandContext<TCommandSource>, playerClass: Class<*>): Int {
    ...
    memberManager.deleteNickName(userService.getUserName(context.source as TPlayer))
      .thenAcceptAsync { textService.send(it, context.source) }
    return 1
  }

  fun executeOther(context: CommandContext<TCommandSource>): Int {
    memberManager.deleteNickNameForUser(context.getArgument("target", String::class.java))
      .thenAcceptAsync { textService.send(it, context.source) }
    return 1
  }
}
