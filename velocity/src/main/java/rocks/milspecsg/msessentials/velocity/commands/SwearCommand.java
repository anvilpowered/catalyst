...
public class SwearCommand implements Command {
    ...
    public void execute(CommandSource source, @NonNull String[] args) {
        ...
        switch (args[0]) {
            case "list": {
                if (source.hasPermission(registry.getOrDefault(MSEssentialsKeys.LANGUAGE_ADMIN)) || source.hasPermission(registry.getOrDefault(MSEssentialsKeys.LANGUAGE_LIST))) {
            ...
            case "add": {
                if (source.hasPermission(registry.getOrDefault(MSEssentialsKeys.LANGUAGE_ADMIN))) {
