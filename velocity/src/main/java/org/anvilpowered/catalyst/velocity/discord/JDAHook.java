...
public class JDAHook {
    ...
    public void registryLoaded() {
        if (registry.getOrDefault(CatalystKeys.DISCORD_ENABLE)) {
            proxyServer.getEventManager().register(
                catalyst,
                catalyst.getPrimaryEnvironment()
                    .getInjector().getInstance(DiscordProxyListener.class));
            try {
            ...
            logger.warn(
                "The discord module is currently disabled! Chat will not be transmitted to discord."
            );
        }
    }
    ...
    public void updateTopic() {
        proxyServer.getScheduler().buildTask(catalyst, () -> {
            ...
            c.getManager().setTopic(
                registry.getOrDefault(CatalystKeys.TOPIC_FORMAT).replace("%players%", playerCount)
            ).queue();
        }).repeat(
            registry.getOrDefault(CatalystKeys.TOPIC_UPDATE_DELAY).longValue(), TimeUnit.SECONDS
        ).schedule();
    }
}
