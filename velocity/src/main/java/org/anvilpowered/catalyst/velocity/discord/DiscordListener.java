...
public class DiscordListener extends ListenerAdapter {
    ...
    public void onMessageReceived(MessageReceivedEvent event) {
        ...
        if (event.getChannel().getId().equals(registry.getOrDefault(CatalystKeys.MAIN_CHANNEL))) {
            if (event.getMessage().toString().contains("!cmd")
                ...
                proxyServer.broadcast(
                    legacyColor(message)
                        .clickEvent(ClickEvent.openUrl(
                            registry.getOrDefault(CatalystKeys.DISCORD_URL)))
                );
            }
        }
        ...
        if (event.getChannel().getId().equals(registry.getOrDefault(CatalystKeys.STAFF_CHANNEL))) {
            ...
            for (Player p : proxyServer.getAllPlayers()) {
                if (p.hasPermission(registry.getOrDefault(CatalystKeys.STAFFCHAT))) {
                    p.sendMessage(legacyColor(message)
                        .clickEvent(ClickEvent.openUrl(
                            registry.getOrDefault(CatalystKeys.DISCORD_URL)))
                        .hoverEvent(HoverEvent.showText(
                            TextComponent.of(
                                registry.getOrDefault(CatalystKeys.DISCORD_HOVER_MESSAGE)))));
                }
            }
        }
    }
