...
class ProxyFormat(
    ...
    class Resolver(private val proxyServer: ProxyServer) {
        def resolve(format: Component, placeholders: Placeholders): Component {
        ...
        def resolve(format: ProxyFormat): Component = resolve(format.format, format.placeholders)
    }

    companion object Builder : MessageFormat.Builder[Placeholders, ProxyFormat] {
        override def build(block: Placeholders.() -] Component): ProxyFormat {
