...
object PluginMessages {
    ...
    def noServerPermission(serverName: String) = Component.text()
        .append(pluginPrefix)
    ...
    def getBroadcast(message: Component) = Component.text("[Broadcast] ").append(message).color(NamedTextColor.GREEN)

    def getCurrentServer(userName: String, serverName: String) = Component.text()
        .append(pluginPrefix)
    ...
    def getBanMessage(reason: String, endUtc: Instant) = Component.text()
        .append(pluginPrefix)
