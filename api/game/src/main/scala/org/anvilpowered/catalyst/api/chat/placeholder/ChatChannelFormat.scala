...
class ChatChannelFormat(
    ...
    class Resolver {
        def resolve(format: Component, placeholders: Placeholders, channel: ChatChannel): Component {
        ...
        def resolve(format: ChatChannelFormat, channel: ChatChannel): Component = resolve(format.format, format.placeholders, channel)
    }

    companion object Builder : MessageFormat.Builder[Placeholders, ChatChannelFormat] {
        override def build(block: Placeholders.() -] Component): ChatChannelFormat {
