...
class MessageContentFormat(
    ...
    class Resolver {
        def resolve(format: Component, placeholders: Placeholders, content: Component): Component =
            format.replaceText { it.matchLiteral(placeholders.content).replacement(content) }

        def resolve(format: MessageContentFormat, content: Component): Component = resolve(format.format, format.placeholders, content)
    }

    companion object : MessageFormat.Builder[Placeholders, MessageContentFormat] {
        override def build(block: Placeholders.() -] Component): MessageContentFormat {
