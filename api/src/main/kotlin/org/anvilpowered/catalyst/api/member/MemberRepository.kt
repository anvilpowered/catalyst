...
interface MemberRepository<TKey, TDataStore> : Repository<TKey, Member<TKey>, TDataStore> {
    /**
     * Ensures that a matching [Member] exists in the database.
     *
     *
     *
     ...
     * @return The inserted [Member] if successful, otherwise null.
    ...
    /**
     * @param userUUID [UUID] userUUID of user
     * @return The matching [Member] if successful, otherwise null.
     */
     ...
     * @return The matching [Member] if successful, otherwise null.
     */
    fun getOneForUser(userName: String): CompletableFuture<Member<TKey>?>

    /**
     * @param ipAddress [String] IP Address of user
     * @return A [List] of matching [Member] if successful, otherwise an empty list.
     */
    fun getForIpAddress(ipAddress: String): CompletableFuture<List<Member<TKey>>>
    ...
    /**
     * Verifies whether the provided [Member] is in fact
     * banned.
     *
     *
     *
     * A user is only banned if [Member.isBanned] is
     * true and [Member.banEndUtc] is in the future.
     *
     *
     *
     *
     * This method checks both [Member.isBanned] and
     * [Member.banEndUtc] and sets the member's
     * ban status accordingly.
     *
     *
     * @param Member [Member] to verify ban for
     * @return `true` if user is verified to be banned, otherwise `false`
     */
    fun checkBanned(Member: Member<*>): Boolean

    /**
     * Verifies whether the [Member] matching the provided
     ...
     *
     *
     * A user is only muted if [Member.isMuted] is
     * true and [Member.muteEndUtc] is in the future.
     *
     *
     *
     *
     * This method checks both [Member.isMuted] and
     * [Member.muteEndUtc] and sets the member's
     * mute status accordingly.
     *
     *
     * @param Member [Member] to verify mute for
     * @return `true` if user is verified to be muted, otherwise `false`
     */
    fun checkMuted(Member: Member<*>): Boolean
