...
class JoinListener<TString, TPlayer, TCommandSource> @Inject constructor(
  ...
  fun onPlayerJoin(event: JoinEvent<TPlayer>) {
    ...
    val joinMessage = if (registry.getOrDefault(CatalystKeys.EMOJI_ENABLE)) {
    ...
    if (registry.getOrDefault(CatalystKeys.JOIN_LISTENER_ENABLED)) {
      broadcastService.broadcast(textService.deserialize(joinMessage.replace("%player%", userName).replace("%server%", server)))
      logger.info(
        textService.serializePlain(
          textService.of(registry.getOrDefault(CatalystKeys.JOIN_MESSAGE).replace("%player%", userName).replace("%server%", server))
        )
      )
    }
  }
