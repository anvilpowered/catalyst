...
class ChannelCommand<TString, TPlayer : TCommandSource, TCommandSource> @Inject constructor(
  ...
  fun editProperty(context: CommandContext<TCommandSource>): Int {
    ...
    if (!ChannelEdit.currentChannel.containsKey(uuid)) {
      textService.send(textService.fail("You must select a channel to edit first!"), source)
      return 0
    }
    ...
    if (edited) {
      textService.builder()
        ...
        .gold().append(context.getArgument<String>("name"))
        .green().append(" to ")
        .gold().append(value)
        .green().append(" for ")
      ...
      textService.builder()
        ...
        .green().append(context.getArgument<String>("name"))
        .yellow().append("\" for ")
